...
class MeetingState:
    ...
    def from_dict(cls,data:Dict[str,Any])->"MeetingState":
        return cls(*(data[k] for k in ("round","last_writer","next_writer","status","completion","last_event")))
...
def next_state(current:MeetingState,writer:str,event_id:str,status:str="proposed")->MeetingState:
    ...
    if status=="need_user_decision": state=MeetingState(current.round+1,writer,writer,"NEED_USER_DECISION","IN_PROGRESS",event_id)
    else:
        other="Doubao" if writer=="GPT" else "GPT"
        state=MeetingState(current.round+1,writer,other,f"WAITING_{other.upper()}","IN_PROGRESS",event_id)
